...
const AuditLogs = () => {
    ...
    const fetchLogs = async () => {
        try {
            ...
            const res = await axios.get(`${API}/audit-logs?${params.toString()}`, getAuthHeader());
        ...
        <>
            <div className="flex w-full flex-col bg-white p-6">
                ...
                <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
                    <h1 className="text-3xl font-bold text-gray-800">ประวัติการเปลี่ยนแปลง</h1>
                    ...
                    {loading ? (
                        ...
                        <table className="min-w-full divide-y divide-gray-200 text-sm">
                            <thead className="bg-gray-100">
                                <tr>
                                    <th className="px-4 py-3 text-left font-medium text-gray-500 uppercase">เวลา</th>
                                    <th className="px-4 py-3 text-left font-medium text-gray-500 uppercase">ตาราง</th>
                                    <th className="px-4 py-3 text-center font-medium text-gray-500 uppercase">การกระทำ</th>
                                    <th className="px-4 py-3 text-right font-medium text-gray-500 uppercase">รหัสแถว</th>
                                    <th className="px-4 py-3 text-left font-medium text-gray-500 uppercase">ผู้ทำ</th>
                                    <th className="px-4 py-3 text-right font-medium text-gray-500 uppercase">ดู</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
                                {logs.map((log) => (
                                    <tr key={log.audit_id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{formatTime(log.changed_at)}</td>
                                        <td className="px-4 py-3 text-gray-800">{log.table_label}</td>
                                        <td className="px-4 py-3 text-center">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${actionBadge(log.action)}`}>
                                                {actionLabel(log.action)}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-right text-gray-700">{log.record_id ?? '—'}</td>
                                        <td className="px-4 py-3 text-gray-600">{log.changed_by_name || '— (ระบบ)'}</td>
                                        <td className="px-4 py-3 text-right">
                                            <button onClick={() => setDetail(log)} className="text-sm text-indigo-600 hover:text-indigo-900">
                                                ดูข้อมูล
                                            </button>
                                        </td>
                                    </tr>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <h3 className="text-sm font-semibold text-gray-700 mb-2">ก่อนแก้ (old)</h3>
                            ...
                            <div>
                                <h3 className="text-sm font-semibold text-gray-700 mb-2">หลังแก้ (new)</h3>
                                <pre className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-xs overflow-x-auto whitespace-pre-wrap">
                        ...
                        <div className="flex justify-end mt-6">
                            <button onClick={() => setDetail(null)} className="px-4 py-2 text-sm bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg transition">
                                ปิด
                            </button>
                        </div>
                    </div>

      ...
      {steps.map((label, index) => {
        ...
        return (
          <div key={label} className="flex-1 flex flex-col items-center relative">
            ...
            {!isLast && (
              <div
                className={`absolute top-4 left-1/2 w-full h-0.5 ${isDone ? 'bg-[#0194F3]' : 'bg-[#CBD5E1]'}`}
              />
            )}
            <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-sm font-black ${circleClass}`}>
              {isDone ? '✓' : stepNumber}
            </div>
            <span className={`mt-1.5 text-xs font-bold text-center ${isActive ? 'text-[#0194F3]' : 'text-[#94A3B8]'}`}>
              {label}
            </span>
          </div>
        );
